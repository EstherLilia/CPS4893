...
欢迎来到这款刺激的躲避球游戏！在这个60秒的挑战中，你将面临两种不同的模式：屈曲躲避和侧曲躲避。游戏的设计旨在测试你的躯干灵活性和反应能力。

首先是屈曲躲避模式。在这个模式中，你需要通过屈曲躯干的动作来躲避飞来的球，只有这样才能得分。屈曲躲避模式分为三个难度阶段。每20秒，躲避球的抛出位置高度会逐渐降低，从头部到颈部再到胸部，考验你的躯干屈曲能力。尽量在规定时间内完成动作，赢得更高的分数！

接下来是侧曲躲避模式。在这个挑战中，只有通过躯干侧曲的动作才能成功躲避球的碰撞区并得分。同样，侧曲躲避模式也分为三个难度阶段。每20秒，躲避球的抛出位置水平坐标将会逐渐靠近中心，考验你的躯干侧曲能力。确保在规定时间内迅速做出动作，赢得更高的分数！

在游戏中，每发射一个球后，你有4秒的时间来巧妙躲避。如果在倒计时结束时未能满足躲避条件，球将爆炸，你不会得分。而如果你成功躲避，球会飞走并为你增加50分。总分上限为750分，挑战自己，看看你能达到多高的分数吧！快来体验这场刺激的躲避球挑战吧！"""
    Menu_window.geometry(f"1280x720")
    Menu_window.resizable(False,False)
    menu_font=font.Font (size=16,family='Times New Roman')
    menu_label = tk.Label(Menu_window, text=Menu_text, justify=tk.CENTER, anchor='center',wraplength=780,font=menu_font)
    menu_label.pack()
...
def play_background_sound():
    pygame.mixer.music.load('D:/Program Files/Desktop/Dodgeball/video/sound_effect.mp3')  # Replace with the path to your sound file
    pygame.mixer.music.set_volume(0.5)  # Adjust the volume as needed
    pygame.mixer.music.play(-1, 0.0)  # Play the background sound in a loop
...
if __name__ == "__main__":
    ...
    # 打开视频文件
    video_cap = cv2.VideoCapture('D:/Program Files/Desktop/Dodgeball/video/sound_effect.mp4')
